...
require_once('lib/github-php-client-master/client/GitHubClient.php');
...
final class MessageCenter
{
	...
	public static function createGithubIssue($data)
	{
		...
		if ($data['type'] == 'entry') {
			...
			// entry type and market entry type
			$entryTypes = [0 => 'Orga', 1 => 'Börse', 2 => 'Event'];
			$offerType = filter_var($data['entryData']['offer'], FILTER_VALIDATE_BOOLEAN) ? "Angebot" : "Gesuch";
			
			// build issue title
			$title_type = $entryTypes[ $data['entryType'] ];
			if( $data['entryType'] == 1 ) $title_type .= ' (' . $offerType . ')';
			$title = $title_type . ' "' . $data['entryData']['name'] . '" von ' . ($data['entryData']['speakerPublic'] ? $data['entryData']['speakerPublic'] : 'Unbekannt');
			...
			$body .= "__" . $title_type . " von " . $data['entryData']['speakerPublic'] . " (" . $data['entryData']['mail'] . "):__\n";
			...
			$body .= "für Kinder: `" . (filter_var($data['entryData']['forChildren'], FILTER_VALIDATE_BOOLEAN) ? "ja" : "nicht explizit") . "`\n";
			
			$body .= "Unterstützer gesucht: `" . (filter_var($data['entryData']['supportWanted'], FILTER_VALIDATE_BOOLEAN) ? "ja" : "nein") . "`\n";
			
			$body .= "Kontaktperson: `" . ($data['entryData']['speakerPublic'] ? $data['entryData']['speakerPublic'] : " ") . "`\n";
			
			$body .= "Sprachen: `" . ($data['entryData']['spokenLanguages'] ? $data['entryData']['spokenLanguages'] : " ") . "`\n";
			
			$body .= "mail: `" . ($data['entryData']['mail'] ? $data['entryData']['mail'] : " ") . "`\n";
			
			$body .= "web: `" . ($data['entryData']['web'] ? $data['entryData']['web'] : " ") . "`\n";
			
			$body .= "facebook: `" . ($data['entryData']['facebook'] ? $data['entryData']['facebook'] : " ") . "`\n";
			
			$body .= "Telefon: `" . ($data['entryData']['phone'] ? $data['entryData']['phone'] : " ") . "`\n";
			
			$body .= "Ortsbezeichnung: `" . ($data['locationData']['placename'] ? $data['locationData']['placename'] : " ") . "`\n";
			
			$body .= "Straße: `" . ($data['locationData']['street'] ? $data['locationData']['street'] : " ") . "`\n";
			
			$body .= "PLZ: `" . ($data['locationData']['zip'] ? $data['locationData']['zip'] : " ") . "`\n";
			
			$body .= "Ort: `" . ($data['locationData']['city'] ? $data['locationData']['city'] : " ") . "`\n";
		...
		// normal feedback
			...
			$title = $data['feedbackData']['author'] . " schreibt ...";

			$body .= "- [ ] geantwortet\n";
			$body .= "- [ ] abgeschlossen\n\n";

			$body .= $data['feedbackData']['author'] . " (" . $data['feedbackData']['mail'] . ") schrieb:\n";

			$body .= "```\n" . $data['feedbackData']['message'] . "\n```" . "\n\n";

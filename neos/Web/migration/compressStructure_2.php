...
// check if child is the only child
// AND check number of translations for each child
while ($child = mysqli_fetch_object($r)) {
    
    // get all children of the parent
    $r4 = sql("SELECT * FROM `ddfa_main_domain_model_marketentry` WHERE `parent_entry_id` = '" . $child->parent_entry_id . "'");
    // echo $r4->num_rows . '<br>';

    if( intval($r4->num_rows) == 1 ) {
    	...
    	echo 'skipped: ' . $child->entry_id . ' (' . $child->name .') > #locations: ' . $r4->num_rows . '<br><br>';
    }
